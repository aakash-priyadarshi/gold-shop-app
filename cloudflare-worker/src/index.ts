...
async function handleUpload(request: Request, env: Env, corsHeaders: HeadersInit): Promise<Response> {
  ...
  // Handle base64 JSON upload
    ...
    const json = await request.json() as { data?: string; filename?: string; type?: string };
    ...
    return new Response(JSON.stringify({ 
      success: false, 
      error: 'Invalid content type. Use multipart/form-data, application/json with base64, or raw image bytes' 
    }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
...
async function handleDelete(key: string, env: Env, corsHeaders: HeadersInit): Promise<Response> {
  try {
    // Delete all variants
...
async function handleServe(key: string, env: Env, corsHeaders: HeadersInit): Promise<Response> {
