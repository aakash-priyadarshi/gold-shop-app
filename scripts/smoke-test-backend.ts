...
const API_URL =
  process.argv[2] || process.env.API_URL || "https://api.orivraa.com";
...
async function testHealthPing(): Promise<{
  passed: boolean;
  message?: string;
}> {
...
async function testMarketRates(): Promise<{
  passed: boolean;
  message?: string;
  response?: unknown;
}> {
  const response = await fetch(
    `${API_URL}/market-rates?region=NP&currency=NPR`,
  );
  const data = await response.json();
...
async function testMarketRatesStatus(): Promise<{
  passed: boolean;
  message?: string;
  response?: unknown;
}> {
...
async function testAuthProtection(): Promise<{
  passed: boolean;
  message?: string;
}> {
