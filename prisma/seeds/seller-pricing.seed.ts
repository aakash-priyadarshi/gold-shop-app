...
const ADDITIONAL_SHOPS = [
  // India - Mumbai shop
  {
    ...
    metalRates: [
      { metalType: 'GOLD_24K', ratePerGramNpr: 6850 }, // ~Rs 5700/g in INR
  ...
  // UAE - Dubai shop
  {
    ...
    shop: {
      ...
      makingChargePercent: 5, // Lower making charge in Dubai
      minOrderValueNpr: 25000,
    },
    metalRates: [
      { metalType: 'GOLD_24K', ratePerGramNpr: 7200 }, // Premium for purity
  ...
  // UK - London shop
  {
    ...
    shop: {
      ...
      makingChargePercent: 15, // Higher making charge for bespoke
  ...
  // Nepal - Pokhara shop (local competitor)
  {
    shopkeeper: {
  ...
  // EU - Germany shop
  {
    ...
    shop: {
      ...
      makingChargePercent: 18, // High quality artisan work
...
const SHOP2_OVERRIDES: PriceOverride[] = [
  // Metal rate overrides (more competitive)
  ...
  // Making charge override (lower than shop1)
  ...
  for (const shopData of ADDITIONAL_SHOPS) {
    ...
    const shop = await prisma.shop.upsert({
      ...
      create: {
        ...
        descriptionNe: shopData.shop.shopNameNe, // Using Nepali name as fallback
        ...
        codEnabled: shopData.shop.country === 'NP', // COD only for Nepal
    ...
    // Create gemstone pricing
    await prisma.shopGemstonePricing.createMany({
      skipDuplicates: true,
      data: shopData.gemstonePricing.map(gem => ({
        shopId: shop.id,
        stoneType: gem.stoneType,
        qualityGrade: gem.qualityGrade,
        pricePerCaratNpr: gem.pricePerCaratNpr,
      })),
    });

    console.log(`   ✓ Created shop with ${shopData.metalRates.length} metal rates, ${shopData.finishPricing.length} finishes, ${shopData.gemstonePricing.length} gemstones`);
  }

  // Add price overrides for existing shops
