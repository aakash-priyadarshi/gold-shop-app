...
For penetration testing, verified fixture identities can be created with
`prisma/seed-pentest.ts` in a disposable `NODE_ENV=test` database only.

The caller must provide a fresh random `SEED_PENTEST_PASSWORD`. No password or
production target is stored in the repository, and the accounts must never be
seeded into production.

---

_Generated as part of security audit for gold-shop-app_
