...
import { MaterialCategory, JewelleryType, GemstoneType, PlatingOption } from '@prisma/client';
...
const PRECIOUS_METALS = [
  ...
  { id: 'PLATINUM_900', name: 'Platinum (PT900)', purity: 0.90 },
...
const JEWELLERY_ALLOYS = [
  ...
  { id: 'NICKEL_ALLOY', name: 'Nickel Alloy (Restricted)', complianceFlags: ['NICKEL_COMPLIANCE_REQUIRED'] },
...
const PLATING_TYPES = [
  ...
  { id: 'ROSE_GOLD_PLATING', name: 'Rose Gold Plating', requiresSilverBase: false },
  ...
  { id: 'VERMEIL', name: 'Vermeil (Gold over Sterling Silver)', requiresSilverBase: true },
  { id: 'PVD_COATING', name: 'PVD Coating', requiresSilverBase: false },
];

const PLATING_TIERS = [
  { id: 'LIGHT', name: 'Light (0.5-1μm)', thicknessRange: '0.5-1 microns', durability: '3-6 months' },
  { id: 'STANDARD', name: 'Standard (1-2.5μm)', thicknessRange: '1-2.5 microns', durability: '6-12 months' },
  { id: 'PREMIUM', name: 'Premium (2.5-5μm)', thicknessRange: '2.5-5 microns', durability: '12-24 months' },
];
...
const SETTING_STYLES = [
  { id: 'PRONG', name: 'Prong Setting', description: 'Classic setting with metal claws holding the stone' },
  { id: 'BEZEL', name: 'Bezel Setting', description: 'Metal rim surrounding the stone' },
  { id: 'CHANNEL', name: 'Channel Setting', description: 'Stones set in a metal channel' },
  { id: 'PAVE', name: 'Pavé Setting', description: 'Multiple small stones set closely together' },
  { id: 'FLUSH', name: 'Flush/Gypsy Setting', description: 'Stone sits flush with metal surface' },
  { id: 'TENSION', name: 'Tension Setting', description: 'Stone held by pressure from band' },
  { id: 'HALO', name: 'Halo Setting', description: 'Center stone surrounded by smaller stones' },
  { id: 'CLUSTER', name: 'Cluster Setting', description: 'Multiple stones grouped together' },
  { id: 'BAR', name: 'Bar Setting', description: 'Metal bars hold stones on sides' },
  { id: 'INVISIBLE', name: 'Invisible Setting', description: 'No metal visible between stones' },
];
...
export class MaterialsService {
  ...
  getBuildMethods() {
    return [
      ...
      },
      {
        ...
        label: 'Multi-metal construction. Not solid gold unless 100% gold selected.',
      },
    ];
  }
  ...
  getCompositionRules() {
    return {
      ...
      METHOD_D: {
        ...
        optional: ['goldPercentByWeight', 'targetGoldWeightG', 'partDimensions'],
      },
    };
  }
  ...
  async calculatePriceEstimate(params: {
    ...
    const { 
      ...
      platingTier,
      gemstones = [],
      makingChargePercent = 12, // Default making charge
    } = params;
    ...
    if (buildMethod === 'METHOD_C' && platingType && platingTier) {
      ...
      const option = platingOptions.find(
        (p: PlatingOption) => p.platingType === platingType && p.tier === platingTier
      );
    ...
    return {
      ...
      labels: buildMethod === 'METHOD_C' ? ['Not solid gold. Plated/Coated.'] : [],
    };
  }
}
