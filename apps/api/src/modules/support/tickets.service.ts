...
export class TicketsService {
  ...
  async createTicket(dto: CreateTicketDto, userId?: string, userRole?: string) {
    ...
    this.logger.log(
      `Ticket ${ticketNumber} created by ${userId || dto.guestEmail || "anonymous"}`,
    );
    return ticket;
  }
  ...
  async claimTicket(ticketId: string, staffId: string) {
    ...
    if (ticket.assigneeId && ticket.assigneeId !== staffId) {
      throw new BadRequestException(
        "This ticket is already claimed by another agent",
      );
    }
    ...
    if (ticket.userId) {
      await this.notifications.create(
        ticketNotif(
          ...
          note ||
            `Your ticket status has been updated to ${status.replace(/_/g, " ")}.`,
          ticket.id,
        ),
      );
    }
  ...
  async getTicketStats() {
    const [open, claimed, inProgress, waitingUser, resolved, total] =
      await Promise.all([
        ...
        this.prisma.supportTicket.count({
          where: { status: TicketStatus.CLAIMED },
        }),
        this.prisma.supportTicket.count({
          where: { status: TicketStatus.IN_PROGRESS },
        }),
        this.prisma.supportTicket.count({
          where: { status: TicketStatus.WAITING_USER },
        }),
        this.prisma.supportTicket.count({
