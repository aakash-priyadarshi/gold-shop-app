...
export class InventoryService {
  ...
  /**
   * Idempotent demo necklace so shopkeepers can compare the POS sheet
   * with /m/products/:id (gallery, HUID, gems, sizes, certificate).
   */
  async createSampleShowProduct(shopId: string, userId: string) {
    const shop = await this.prisma.shop.findFirst({
      where: { id: shopId, userId },
    });
    if (!shop) {
    ...
    const sku = `DEMO-SHOW-${shopId}`;
    const metalValue = 185000;
    const makingCharge = 18500;
    const gemstoneValue = 42000;
    const tax = 3682.5;
    const totalPrice = metalValue + makingCharge + gemstoneValue + tax;
    const gemstones = [
      {
    ...
    const images = [
      "https://images.orivraa.com/product/1769445053991-4ytp6cgd.png",
      "https://images.orivraa.com/product/1769445056509-on5a83b5.png",
      "https://images.orivraa.com/product/1769445051626-dm5ke087.png",
    ];
    const descriptionEn =
      "Sample bridal necklace for the Show to customer screen. 22K gold with a GIA diamond and a GRS ruby. Open this from POS, then tap Show full details to customer to see the gallery, HUID, assay office, gemstone cards, sizes, and certificate — fields the compact POS sheet hides when they are empty.";

    const data = {
      nameEn: "Demo bridal necklace (sample)",
      descriptionEn,
      jewelleryType: JewelleryType.NECKLACE,
      buildMethod: "METHOD_A",
      composition: composition as Prisma.InputJsonValue,
      totalWeightGrams: 28.4,
      gemstones: { gemstones } as Prisma.InputJsonValue,
      metalValueNpr: metalValue,
      makingChargeNpr: makingCharge,
      wastagePercent: 8,
      gemstoneValueNpr: gemstoneValue,
      ...
      totalPriceNpr: totalPrice,
      images,
      certificateUrl: images[1],
      hallmarkNumber: "HUID-DEMO22K9X8Y7",
      assayOffice: AssayOffice.BIRMINGHAM,
      purityCertUrl: images[2],
      labels: ["demo", "sample-show"],
      stockQuantity: 3,
      hasSizes: true,
    ...
    if (existing) {
      ...
      const created = await this.prisma.inventoryItem.create({
        data: { shopId, sku, ...data },
      });
      itemId = created.id;
    }

    const sizes = [
      { label: "16 inch", value: 16, extra: 0 },
      { label: "18 inch", value: 18, extra: 4500 },
      { label: "20 inch", value: 20, extra: 8200 },
    ];
    for (const size of sizes) {
      const variantSku = `${sku}-${size.value}IN`;
      await this.prisma.productVariant.upsert({
        ...
        create: {
          ...
          sku: variantSku,
          stock: 1,
          priceOverride: size.extra ? totalPrice + size.extra : null,
          isActive: true,
        },
        update: {
          inventoryItemId: itemId,
