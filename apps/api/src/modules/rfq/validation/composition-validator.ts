...
function validateMethodA(
  ...
  if (
    ...
    !validMetals.includes(composition.preciousMetal)
  ) {
    errors.push({
    ...
    if (alloyConfig.baseMetal === "GOLD") {
      ...
      if (!alloyConfig.recipePresetId) {
        warnings.push({
          ...
          message:
            "No specific alloy recipe selected. Shop will use standard recipe.",
  ...
  if (hasStandardAlloy && !hasAlloyConfig) {
    ...
    if (!validAlloys.includes(composition.standardAlloy)) {
      errors.push({
        ...
        field: "standardAlloy",
        message:
          "Only preset standard alloys are allowed. Custom percentages not permitted.",
...
function validateMethodC(
  ...
  if (
    composition.coreMetal &&
    !validCoreMetals.includes(composition.coreMetal)
  ) {
    errors.push({
  ...
  if (
    composition.platingTier &&
    !validTiers.includes(composition.platingTier)
  ) {
    errors.push({
...
function validateMethodD(
  ...
  const validPatterns: MultiMetalPattern[] = [
    "TOP_PLATE",
    "INLAY",
    "OUTER_SLEEVE",
    "TWO_TONE_SPLIT",
  ...
  if (
    composition.multiMetalPattern &&
    !validPatterns.includes(composition.multiMetalPattern)
  ) {
    errors.push({
  ...
  if (
    composition.multiMetalMode &&
    !validModes.includes(composition.multiMetalMode)
  ) {
    errors.push({
...
function validateModeD1(config: any, errors: ValidationError[]): void {
  if (config.goldPercentByWeight === undefined) {
    ...
    config.goldPercentByWeight < 10 ||
    config.goldPercentByWeight > 90
  ) {
    errors.push({
