...
export class ShopQuotesController {
  ...
  @Public()
  @ApiOperation({
    summary: "Public: track quote/order by token (no auth required)",
  })
  async trackByToken(@Param("token") token: string) {
    return this.shopQuotesService.getByTrackingToken(token);
  }
  ...
  @ApiOperation({
    ...
    description:
      "Uses Redis cache for fast lookup. Returns customer details and recent orders if found.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Returns up to 5 matching customers for live suggestions as the seller types.",
  })
  ...
  async getCustomerHistory(
    ...
    return this.shopQuotesService.getCustomerHistory(
      customerId,
      shopId,
      userId,
    );
  ...
  @Roles(UserRole.SHOPKEEPER)
  @ApiOperation({
    summary: "Update quote status (confirm, start, complete, cancel)",
  })
  async updateStatus(
    @Param("id") id: string,
    @CurrentUser("shopId") shopId: string,
  ...
  @Roles(UserRole.SHOPKEEPER)
  @ApiOperation({
    summary: "Convert a quote to an invoice (generates invoice number)",
  })
  async convertToInvoice(
    ...
    return this.shopQuotesService.convertToInvoice(id, shopId, userId, dto);
  }

  @Post(":id/send-tracking-link")
  @Roles(UserRole.SHOPKEEPER)
  @ApiOperation({
    summary: "Send the customer their tracking link via email or SMS",
  })
  async sendTrackingLink(
    ...
    return this.shopQuotesService.sendTrackingNotification(
      id,
      shopId,
      userId,
      dto,
