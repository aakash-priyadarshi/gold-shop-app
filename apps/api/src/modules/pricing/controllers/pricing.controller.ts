...
import { PricingEngineService, SupportedCountry } from '../services/pricing-engine.service';
import { CommodityRatesService, PURITY_MULTIPLIERS } from '../services/commodity-rates.service';
import { TaxRulesService, DEFAULT_TAX_RATES } from '../services/tax-rules.service';
import { BackendTaxEngineService, TaxableComponent, TaxCalculationResult } from '../services/backend-tax-engine.service';
...
export class PricingController {
  ...
  @ApiOperation({
    ...
    description: 'Calculate comprehensive pricing with materials, taxes, and explainable breakdown',
  })
  @ApiResponse({
    status: 200,
  ...
  async quickEstimate(
    ...
    const purityMultiplier = PURITY_MULTIPLIERS[request.metalCode as keyof typeof PURITY_MULTIPLIERS] || 1.0;
  ...
  @ApiOperation({
    ...
    description: 'Get available markets, currencies, metals, and other pricing options',
  })
  @ApiResponse({
    status: 200,
  ...
  async getConfig(): Promise<PricingConfigResponseDto> {
    ...
    const preciousMetals = [...PRECIOUS_METAL_CODES].map(code => {
      ...
      return {
        ...
        metalType,
        purityName: purity,
        multiplier: PURITY_MULTIPLIERS[code as keyof typeof PURITY_MULTIPLIERS] || 1.0,
      };
    });
    ...
    const gemstoneTypes = [...STONE_TYPES].map(code => ({
      ...
      availableOrigins: code.includes('DIAMOND') ? [...GEM_ORIGINS] : ['NATURAL'],
    }));
    ...
    const buildMethods = [
      {
        ...
        description: 'Pure precious metal construction (gold, silver, platinum, palladium)',
      },
      {
        code: 'METHOD_B',
        name: 'Base Metal/Alloy',
        description: 'Base metal or alloy construction (brass, bronze, copper, etc.)',
      },
      {
        code: 'METHOD_C',
        name: 'Core + Finish',
      ...
      },
      {
        code: 'METHOD_D',
        name: 'Multi-Metal',
        description: 'Mixed construction with multiple metals (e.g., gold with brass accents)',
      },
    ];
  ...
  async updateMarketAdjustment(
    ...
    const existing = await (this.prisma as any).marketAdjustmentConfig?.findFirst({
      where: {
  ...
  async calculateTax(
    ...
    this.logger.log(`Tax calculation: region=${request.region}, components=${request.components.length}`);

    // Validate region
    const validRegions = ['NP', 'IN', 'AE', 'UK', 'EU', 'US'];
    if (!validRegions.includes(request.region)) {
      throw new Error(`Invalid region: ${request.region}. Must be one of: ${validRegions.join(', ')}`);
    }
  ...
  @ApiOperation({
    ...
    description: 'Get applicable tax types and rates for a market region (for display only)',
  })
  @ApiQuery({
    name: 'region',
    required: true,
