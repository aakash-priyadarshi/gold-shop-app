...
const CHUNKS: { topic: string; content: string }[] = [
  ...
  },
  {
    ...
    content:
      "Orivraa seller referrals pay the referring shop a configured share of the referred shop's gross paid subscription invoice (default 10%, admin-adjustable) while that shop remains subscribed. Commission first becomes credit against the referrer's next Orivraa subscription invoice. For eligible leftover balance, save bank details in Dashboard → Referrals and request a bank payout; the request remains pending until Orivraa processes it, and a rejected request returns the balance to the referral wallet. Eligible leftover can instead be converted to Pro months. Referral rewards are not AI credits or a refund to the referred shop. Share https://www.orivraa.com/auth/register?ref=CODE or send an email invite from Dashboard → Referrals. Review & Earn is separate: after an admin verifies one supported public platform review, it grants one month of Pro.",
  },
  {
