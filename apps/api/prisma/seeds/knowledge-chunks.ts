...
const CHUNKS: { topic: string; content: string }[] = [
  ...
  },
  {
    ...
    content:
      "Admin → Customer Recovery manages the account-bound 50-day Pro win-back campaign. Admins can preview eligible shopkeepers, including current Pro shops and unverified emails, then filter by country, activity, or account type, select recipients, and send immediately or at the next country-local 10 AM. Inactive accounts, missing shops, and repeat campaign sends stay excluded. Claiming starts complimentary Pro or extends existing Pro to 50 days from the click date; if more than 50 days already remain, the current end date is left unchanged. Unverified recipients sign in, confirm their email code, then activate the offer. The campaign funnel shows sent, delivered, unique opened, unique clicked, claimed Pro, and rejoined counts plus country breakdowns. Open tracking is approximate because privacy tools and blocked images affect the tracking pixel. Rejoined means the recipient had authenticated Orivraa activity or claimed the offer after the email was sent. Delivery, open, click, bounce, complaint, failure, and suppression events come from signature-verified, deduplicated Resend webhooks at /api/recovery-offers/webhooks/resend; enable Resend domain open/click tracking and configure RESEND_WEBHOOK_SECRET before relying on those metrics.",
  },
  {
