...
const CHUNKS: { topic: string; content: string }[] = [
  ...
  },
  {
    ...
    content:
      "Billing wastage (also called jarti) is the customer-facing manufacturing-loss charge on Create Invoice — separate from karigar workshop wastage limits in Supply Chain. Product Catalog pieces can store a default Wastage %; when you Add from catalog on an invoice, that % is applied automatically. On the invoice you can Adjust (+/- %) — e.g. catalog 5% +1 becomes 6% — then amounts recalculate. You can also click Calculate wastage for all lines. Hover How is this calculated? for the formula tooltip. Weight % mode: (1) wastage grams = net weight × (wastage % ÷ 100); (2) rate per gram = metal cost ÷ net weight; (3) wastage amount = wastage grams × rate. Metal value % mode: wastage = metal cost × %. Printed bills show line breakdown plus Incl. wastage / Incl. making and tax detail (metal/wastage/making/gemstone). Change shop-wide defaults under Shop Settings → Preferences → Billing Wastage.",
  },
];
