...
const CHUNKS: { topic: string; content: string }[] = [
  ...
  },
  {
    ...
    content:
      "Billing wastage (also called jarti) is the customer-facing manufacturing-loss charge on Create Invoice — separate from karigar workshop wastage. Three pipelines: (1) Catalog — each product has a required Wastage % (can be 0). Add from catalog fetches that % onto the invoice. One editable % field; caption shows e.g. '5% from catalog' and if you raise to 6% shows '+1% adjusted' plus the price — no separate adjust input. (2) Walk-in quote — set wastage when Mark Ready (built); it carries to the final invoice as 'from walk-in ready'. If left 0, change it on the invoice. (3) Manual invoice — enter % and Calculate only; nothing is fetched. Weight % mode: wastage grams = net weight × (wastage % ÷ 100); amount = wastage grams × (metal cost ÷ net weight). Metal value % mode: wastage = metal cost × %. Shop defaults: Settings → Preferences → Billing Wastage.",
  },
];
