...
const CHUNKS: { topic: string; content: string }[] = [
  ...
  },
  {
    ...
    content:
      "Admin → Customer Recovery manages the account-bound 50-day Pro win-back campaign. Every shopkeeper is eligible except suspended, deactivated, and already-claimed offers. Pending-verification accounts (including Google/OTP users who have not finished shop setup), unverified emails, shops that are inactive or missing, paid Pro shops, and unclaimed queued offers can all be selected. Nothing is selected until the admin checks one email or Select all visible. Send immediately, at the next country-local 10 AM, or at a custom date/time; each row also has an editable send time that overrides the campaign schedule for that email. Re-sending an unclaimed offer replaces the previous token and queue job. Claiming starts complimentary Pro or extends existing Pro to 50 days from the click date; if more than 50 days already remain, the current end date is left unchanged. Recipients whose email is not verified sign in, confirm their email code, then activate the offer; pending-verification shopkeepers are marked active after a successful claim. The campaign funnel shows sent, delivered, unique opened, unique clicked, claimed Pro, and rejoined counts plus country breakdowns. Open tracking is approximate because privacy tools and blocked images affect the tracking pixel. Rejoined means the recipient had authenticated Orivraa activity or claimed the offer after the email was sent. Delivery, open, click, bounce, complaint, failure, and suppression events come from signature-verified, deduplicated Resend webhooks at /api/recovery-offers/webhooks/resend; enable Resend domain open/click tracking and configure RESEND_WEBHOOK_SECRET before relying on those metrics.",
  },
  {
