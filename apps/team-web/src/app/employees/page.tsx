...
export default function EmployeesPage() {
  ...
  const [form, setForm] = useState({
    firstName: "", lastName: "", email: "", phone: "", role: "SALES",
  ...
  const handleCreate = async () => {
    try {
      ...
      setShowAdd(false);
      setForm({ firstName: "", lastName: "", email: "", phone: "", role: "SALES", departmentId: "", baseSalary: "" });
      load();
    } catch {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={form.role} onValueChange={(v) => setForm({ ...form, role: v })}>
                    ...
                    <SelectContent>
                      {["SALES", "SUPPORT", "MANAGER", "ADMIN", "CRAFTSMAN", "DELIVERY"].map((r) => (
              ...
              {employees.map((emp: any) => (
                <Card key={emp.id}>
                  <CardContent className="p-4">
                    ...
                    {emp.isActive !== false && (
                      <Button variant="ghost" size="sm" className="mt-2 text-destructive" onClick={() => handleTerminate(emp.id)}>
        ...
        <CardContent>
          {summary ? (
            <div className="grid grid-cols-3 gap-4 text-center">
              <div><p className="text-2xl font-bold text-emerald-500">{summary.present ?? 0}</p><p className="text-sm text-muted-foreground">Present</p></div>
              <div><p className="text-2xl font-bold text-amber-500">{summary.late ?? 0}</p><p className="text-sm text-muted-foreground">Late</p></div>
              <div><p className="text-2xl font-bold text-red-500">{summary.absent ?? 0}</p><p className="text-sm text-muted-foreground">Absent</p></div>
            </div>
          ) : (
...
function LeavesTab() {
  ...
  const handleApprove = async (id: string) => {
    ...
    employeeApi.listLeaves().then((res) => setLeaves(Array.isArray(res.data) ? res.data : res.data?.data ?? []));
  };
          ...
          <Card key={l.id}>
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{l.type} — {l.startDate} to {l.endDate}</p>
            ...
            <tbody>
              {payroll.map((p: any) => (
                <tr key={p.id} className="border-b">
                  ...
                  <td className="p-3 text-right">{p.bonus?.toLocaleString() ?? 0}</td>
                  <td className="p-3 text-right">{p.deductions?.toLocaleString() ?? 0}</td>
