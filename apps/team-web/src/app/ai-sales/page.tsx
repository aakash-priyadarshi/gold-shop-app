...
import { Plus, Phone, PhoneOff, Users, TrendingUp, Bot } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Manage AI agents, leads, and call campaigns</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent><p className="text-2xl font-bold">{callStats?.totalCalls ?? 0}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent><p className="text-2xl font-bold">{callStats?.conversionRate ?? "0"}%</p></CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="leads">
                ...
                return (
                  <div key={stage} className="flex-1 min-w-[200px]">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant={STAGE_COLORS[stage] as any}>{stageLeads.length}</Badge>
                    </div>
        ...
        <TabsContent value="agents" className="space-y-4">
          {agents.length === 0 ? (
            <Card><CardContent className="py-12 text-center text-muted-foreground">No AI agents configured</CardContent></Card>
          ) : (
                ...
                <Card key={agent.id}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={agent.isActive ? "success" : "secondary"}>{agent.isActive ? "Active" : "Inactive"}</Badge>
                    </div>
                    {agent.description && <p className="text-sm text-muted-foreground mt-2">{agent.description}</p>}
...
function CallsTab() {
  ...
  return calls.length === 0 ? (
    <Card><CardContent className="py-12 text-center text-muted-foreground">No calls recorded yet</CardContent></Card>
  ) : (
        ...
        <tbody>
          {calls.map((call: any) => (
            <tr key={call.id} className="border-b">
              ...
              <td className="p-3 text-center"><Badge variant={call.status === "COMPLETED" ? "success" : call.status === "FAILED" ? "destructive" : "warning"}>{call.status}</Badge></td>
              <td className="p-3 text-center">{call.duration ? `${Math.round(call.duration / 60)}m` : "—"}</td>
