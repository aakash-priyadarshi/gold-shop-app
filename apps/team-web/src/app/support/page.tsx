...
import { Plus, Search, MessageSquare, BookOpen, Zap } from "lucide-react";
...
const STATUS_COLORS: Record<string, string> = {
  OPEN: "warning", IN_PROGRESS: "default", WAITING: "secondary",
...
export default function SupportPage() {
  ...
  const [form, setForm] = useState({ subject: "", description: "", priority: "MEDIUM", channel: "EMAIL", customerName: "", customerEmail: "" });
  ...
  const load = () => {
    ...
    Promise.allSettled([supportApi.listTickets(params), supportApi.getDashboard()])
      .then(([ticketRes, dashRes]) => {
        setTickets(ticketRes.status === "fulfilled" ? (Array.isArray(ticketRes.value.data) ? ticketRes.value.data : ticketRes.value.data?.data ?? []) : []);
  ...
  const handleCreate = async () => {
    try {
      ...
      setShowCreate(false);
      setForm({ subject: "", description: "", priority: "MEDIUM", channel: "EMAIL", customerName: "", customerEmail: "" });
      load();
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Customer support tickets and knowledge base</p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={form.channel} onValueChange={(v) => setForm({ ...form, channel: v })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {["EMAIL", "PHONE", "CHAT", "SOCIAL", "WALK_IN"].map((c) => (
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent><p className="text-2xl font-bold text-amber-500">{dashboard?.openTickets ?? 0}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Resolved Today</CardTitle></CardHeader>
          <CardContent><p className="text-2xl font-bold text-emerald-500">{dashboard?.resolvedToday ?? 0}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Avg Response Time</CardTitle></CardHeader>
          <CardContent><p className="text-2xl font-bold">{dashboard?.avgResponseTime ?? "—"}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Satisfaction</CardTitle></CardHeader>
          <CardContent><p className="text-2xl font-bold">{dashboard?.satisfaction ?? "—"}%</p></CardContent>
        </Card>
      </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                          ...
                          {ticket.assignee && <span>→ {ticket.assignee.firstName} {ticket.assignee.lastName}</span>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 ml-4">
                        <Badge variant={STATUS_COLORS[ticket.status] as any}>{ticket.status?.replace(/_/g, " ")}</Badge>
                        <div className="flex gap-1">
                          {ticket.status === "OPEN" && (
                            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={async () => { await supportApi.resolveTicket(ticket.id); load(); }}>
                              Resolve
                            </Button>
                          )}
                          {ticket.status === "RESOLVED" && (
                            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={async () => { await supportApi.closeTicket(ticket.id); load(); }}>
                              Close
                            </Button>
                          )}
                        </div>
...
function KnowledgeTab() {
  ...
  useEffect(() => {
    supportApi.listArticles(search ? { search } : undefined).then((res) => {
...
function CannedTab() {
  ...
  useEffect(() => {
    supportApi.listCannedResponses().then((res) => {
          ...
          {responses.map((resp: any) => (
            <Card key={resp.id}>
              <CardContent className="p-4">
                ...
                <p className="text-sm text-muted-foreground mt-2 line-clamp-3">{resp.content}</p>
