  ...
  const load = () => {
    ...
    Promise.all([certificateApi.list(params), certificateApi.listTemplates()])
      .then(([certRes, tplRes]) => {
        ...
        setTemplates(
          Array.isArray(tplRes.data) ? tplRes.data : (tplRes.data?.data ?? []),
        );
      })
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Issue and manage certificates with QR verification
          </p>
        </div>
                ...
                <Card key={tpl.id}>
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between mt-3 text-xs text-muted-foreground">
                      ...
                      <Badge
                        variant={
                          tpl.isActive !== false ? "success" : "secondary"
                        }
                      >
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2">
          <Input
            placeholder="Enter QR token or certificate number..."
            value={token}
        ...
        {result && (
          <div
            className={`p-4 rounded-lg border ${result.valid !== false ? "border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950" : "border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950"}`}
          >
            <div className="flex items-center gap-2">
              {result.valid !== false ? (
                <>
            ...
            {result.certificate && (
              <div className="mt-2 text-sm space-y-1">
                ...
                <p>
                  Issued:{" "}
                  {new Date(
                    result.certificate.issuedAt ?? result.certificate.createdAt,
                  ).toLocaleDateString()}
                </p>
              </div>
            )}
