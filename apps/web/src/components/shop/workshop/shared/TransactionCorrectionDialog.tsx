...
export function TransactionCorrectionDialog({
  ...
  const handleSubmit = async () => {
    if (!reason.trim()) {
      setErrorMessage("A detailed audit reason is strictly required for physical transaction corrections");
      return;
    }
    if (correctionMode === "REPLACE" && (!replacementWeight || parseFloat(replacementWeight) <= 0)) {
      setErrorMessage("Enter a valid positive replacement weight in grams");
      return;
    }
    ...
    try {
      ...
      setErrorMessage(
        err?.response?.data?.message || err?.message || "Correction failed. Check dependency requirements."
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground font-mono">
                #{journal.entryNumber} · <T>{journal.referenceType}</T> · {journal.id.slice(0, 8)}…
              </p>
            </div>
          </div>
          ...
          <div className="rounded-lg border bg-muted/30 p-3 space-y-2 text-xs">
            ...
            <div className="grid grid-cols-2 gap-2 font-mono">
              <div>
                ...
                <span className="font-semibold text-foreground"><T>{journal.referenceType}</T></span>
              </div>
              <div>
            ...
            {journal.description && (
              <div className="text-[11px] text-muted-foreground pt-1 border-t">
                {/* i18n-user-content: stored audit description includes operator-entered reasons and references */ journal.description}
              </div>
            )}
          </div>
