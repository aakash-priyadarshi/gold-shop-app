  ...
  return <Card><CardHeader><CardTitle><T>Factory manager controls</T></CardTitle></CardHeader><CardContent className="space-y-5 text-sm">
    ...
    <section className="space-y-2"><h3 className="font-semibold"><T>Materials and process route</T></h3>
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <label><T>Material kind</T><select className={field} value={materialKind} onChange={(event) => setMaterialKind(event.target.value)}>{["SOLDER", "ALLOY", "RECOVERED", "REFINERY", "DIAMOND", "STONE", "OTHER"].map((kind) => <option key={kind} value={kind}>{t(kind)}</option>)}</select></label>
        ...
        <label><T>Job route step</T><select className={field} value={stepId} onChange={(event) => setStepId(event.target.value)}><option value="">{t("Select step")}</option>{steps.map((step) => <option key={step.id} value={step.id}>{step.position + 1}. {definitions.find((definition) => definition.id === step.definitionId)?.name} · {t(step.status)}</option>)}</select></label>
        <label><T>Change</T><select className={field} value={stepAction} onChange={(event) => setStepAction(event.target.value as typeof stepAction)}>{["SKIP", "REPEAT", "REWORK", "ADD"].map((value) => <option key={value} value={value}>{t(value)}</option>)}</select></label>
    ...
    <section className="space-y-2 border-t pt-4"><h3 className="font-semibold"><T>Pieces, machines and transfer tolerance</T></h3>
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <label><T>Granularity</T><select className={field} value={childKind} onChange={(event) => setChildKind(event.target.value as typeof childKind)}>{["DESIGN_GROUP", "ORDER_GROUP", "PIECE"].map((kind) => <option key={kind} value={kind}>{t(kind)}</option>)}</select></label>
    ...
    <section className="space-y-2 border-t pt-4"><h3 className="font-semibold"><T>Assay and audited corrections</T></h3>
      ...
      <div className="grid gap-2 md:grid-cols-4">
        <label><T>Physical source account</T><select className={field} value={sourceAccountId} onChange={(event) => setSourceAccountId(event.target.value)}><option value="">{t("Select account")}</option>{accounts.filter((account) => account.balanceGrams !== "0.000000" && !account.balanceGrams.startsWith("-") && ["VAULT", "WIP", "PROCESS", "REUSABLE", "SCRAP", "RECOVERY_PENDING", "REFINERY", "FINISHED"].includes(account.bucket)).map((account) => <option key={account.id} value={account.id}>{account.materialKey} · {t(account.bucket)} · {account.balanceGrams} g</option>)}</select></label>
        <label><T>Destination</T><select className={field} value={destinationBucket} onChange={(event) => setDestinationBucket(event.target.value)}>{["VAULT", "WIP", "PROCESS", "REUSABLE", "SCRAP", "RECOVERY_PENDING", "REFINERY", "FINISHED"].map((bucket) => <option key={bucket} value={bucket}>{t(bucket)}</option>)}</select></label>
