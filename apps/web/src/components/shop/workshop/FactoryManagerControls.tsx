...
export function FactoryManagerControls({ materials, definitions, routes, accounts, jobId, treeId, runId, steps, onRefresh }: {
  ...
  const postManual = async () => {
    const payload = { materialKey: source?.materialKey || "goldGrains995", sourceBucket: source?.bucket || "VAULT", sourceScopeId: source?.scopeId || "SHOP",
  ...
  return <Card><CardHeader><CardTitle><T>Factory manager controls</T></CardTitle></CardHeader><CardContent className="space-y-5 text-sm">
    ...
    <section className="space-y-2"><h3 className="font-semibold"><T>Materials and process route</T></h3>
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <div className="flex gap-2"><Button disabled={busy || !routeName.trim() || !routeDefinitionIds.length} onClick={() => act(() => workshopApi.createRoute({ name: routeName.trim(), stepDefinitionIds: routeDefinitionIds }), "Route created")}><T>Create route</T></Button><Button variant="outline" onClick={() => setRouteDefinitionIds([])}><T>Clear steps</T></Button></div>
        ...
        <Button disabled={busy || !jobId || !stepId || !stepReason.trim() || (stepAction === "ADD" && !addedDefinitionId)} onClick={() => act(() => workshopApi.changeRouteStep(jobId, stepId, { action: stepAction as any, reason: stepReason.trim(), ...(stepAction === "ADD" ? { definitionId: addedDefinitionId } : {}) }), "Route step changed")}><T>Apply route change</T></Button>
      </div>
    </section>
    <section className="space-y-2 border-t pt-4"><h3 className="font-semibold"><T>Pieces, machines and transfer tolerance</T></h3>
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <Button disabled={busy || !treeId || !childLabel.trim()} onClick={() => act(() => workshopApi.createBatchChild({ treeId, kind: childKind as any, label: childLabel.trim(), quantity: Number(childQuantity) }), "Batch child created")}><T>Add piece or group</T></Button>
