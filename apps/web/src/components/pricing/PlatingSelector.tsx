...
import { HelpCircle, Info, Shield, Droplet, Clock, User, AlertTriangle } from 'lucide-react';
import { PLATING_TYPES, PLATING_TIERS, type PlatingType, type PlatingTier } from '@/lib/pricing/constants';
...
export function PlatingSelector({
  ...
  const selectedPlatingInfo = PLATING_TYPES.find(p => p.value === platingType);
  ...
  const isVermeilInvalid = platingType === 'VERMEIL' && baseMetalType && !baseMetalType.includes('SILVER_925');
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-[300px]">
                <p className="text-xs">
                  Plating adds a thin layer of precious metal over the base metal. 
                  Different types have different appearances, durability, and costs.
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
              ...
              <Popover key={type.value}>
                <PopoverTrigger asChild>
                  <button
                    type="button"
                    disabled={disabled}
                    onClick={() => onPlatingTypeChange(type.value as PlatingType)}
                    className={`
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <Tooltip>
                ...
                <TooltipContent className="max-w-[300px]">
                  <p className="text-xs">
                    Thicker plating lasts longer but costs more. 
                    Premium is best for daily wear items.
                  </p>
                </TooltipContent>
              </Tooltip>
            </div>
                ...
                <Tooltip key={tier.value}>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      disabled={disabled}
                      onClick={() => onPlatingTierChange(tier.value as PlatingTier)}
                      className={`
                        relative flex flex-col items-start p-3 rounded-lg border-2 text-left transition-all
                    ...
                    >
                      ...
                      {tier.priceMultiplier !== 1.0 && (
                        <span className={`text-xs mt-1 ${tier.priceMultiplier > 1 ? 'text-amber-600' : 'text-green-600'}`}>
                          {tier.priceMultiplier > 1 ? `+${((tier.priceMultiplier - 1) * 100).toFixed(0)}%` : `${((tier.priceMultiplier - 1) * 100).toFixed(0)}%`}
                        </span>
                      )}
                    </button>
                  </TooltipTrigger>
        ...
        {selectedPlatingInfo && selectedTierInfo && (
          <div className="bg-gray-50 border rounded-lg p-3 text-sm">
            ...
            <p className="text-muted-foreground">
              <span className="font-medium text-foreground">{selectedPlatingInfo.label}</span>
              {' '}with{' '}
              <span className="font-medium text-foreground">{selectedTierInfo.label}</span> thickness
            </p>
