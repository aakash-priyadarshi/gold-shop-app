      ...
      <div className="bg-gradient-to-br from-amber-50 to-yellow-50 border border-amber-200 rounded-lg p-4 space-y-4">
        ...
        {marketRatesLoading && (
          <div className="flex items-center justify-center py-6">
            ...
            <span className="ml-2 text-sm text-amber-700">Loading market rates...</span>
          </div>
        )}
        ...
        <div className="text-xs text-muted-foreground border-t pt-2 space-y-1">
          ...
          {marketRates?.updatedAt && (
            <p className="flex items-center gap-1">
              ...
              <span>Last fetched on: {formatLastFetched(marketRates.updatedAt)}</span>
            </p>
          )}
          <p className="flex items-center gap-1">
...
function MarketRatesDisplay({ buildMethod, marketRates, currencySymbol }: MarketRatesDisplayProps) {
  const metals = marketRates.metals;
  
  return (
    <div className="space-y-3">
      {/* Method A & B: Show precious metal rates */}
      {(buildMethod === 'METHOD_A' || buildMethod === 'METHOD_B') && (
        <>
          {/* Gold rates */}
          <div>
            <p className="text-xs font-medium text-amber-800 mb-1">Gold (per gram)</p>
            <div className="grid grid-cols-3 gap-1 text-xs">
              {buildMethod === 'METHOD_A' && (
                <RateBox label="24K" value={metals.GOLD_24K} symbol={currencySymbol} />
              )}
              <RateBox label="22K" value={metals.GOLD_22K} symbol={currencySymbol} />
              <RateBox label="18K" value={metals.GOLD_18K} symbol={currencySymbol} />
              {buildMethod === 'METHOD_B' && (
                <>
                  <RateBox label="14K" value={metals.GOLD_14K} symbol={currencySymbol} />
                  <RateBox label="10K" value={metals.GOLD_10K} symbol={currencySymbol} />
                </>
              )}
            </div>
          </div>
          ...
          {/* Silver rates */}
          <div>
            <p className="text-xs font-medium text-amber-800 mb-1">Silver (per gram)</p>
            <div className="grid grid-cols-2 gap-1 text-xs">
              <RateBox label="999" value={metals.SILVER_999} symbol={currencySymbol} color="gray" />
              <RateBox label="925" value={metals.SILVER_925} symbol={currencySymbol} color="gray" />
            </div>
          </div>
          
          {/* Platinum (Method A only) */}
          {buildMethod === 'METHOD_A' && (
            <div>
              <p className="text-xs font-medium text-amber-800 mb-1">Platinum (per gram)</p>
              <div className="grid grid-cols-2 gap-1 text-xs">
                <RateBox label="PT950" value={metals.PLATINUM_PT950} symbol={currencySymbol} color="slate" />
                <RateBox label="PT900" value={metals.PLATINUM_PT900} symbol={currencySymbol} color="slate" />
              </div>
            </div>
          )}
          
          {/* Alloy Premium Note (Method B) */}
          {buildMethod === 'METHOD_B' && (
      ...
      {buildMethod === 'METHOD_C' && (
        <>
          <div className="bg-white/60 rounded p-3 space-y-2">
            <p className="text-xs font-medium text-blue-800">Base Metal + Plating</p>
            ...
            <p className="text-[10px] text-muted-foreground italic">
              Plating is a <strong>service charge</strong> (per piece), not metal weight.
  ...
  return (
    <div className="bg-white/60 rounded px-2 py-1">
      ...
      <span className="font-semibold">
        {symbol} {value?.toLocaleString(undefined, { maximumFractionDigits: 0 }) || 'N/A'}
      </span>
    </div>
  );
}
    ...
    <div className="bg-white rounded-lg p-3 space-y-2 border border-amber-100">
      <div className="flex items-center gap-2 pb-2 border-b">
        ...
        <span className="font-medium text-sm">
          {estimate.status === 'complete' ? 'Estimate Ready' : 'Partial Estimate'}
        </span>
      </div>
      ...
      {estimate.subtotal > 0 && (
        <div className="pt-2 border-t space-y-1 text-sm">
          <div className="flex justify-between">
            ...
            <span>{currencySymbol} {estimate.subtotal.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </div>
          {estimate.taxAmount > 0 && (
            <div className="flex justify-between text-muted-foreground">
              <span>Tax ({(estimate.taxRate * 100).toFixed(0)}%)</span>
              <span>{currencySymbol} {estimate.taxAmount.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
            </div>
          )}
          <div className="flex justify-between font-bold text-lg pt-1 border-t">
            <span>Total</span>
            <span className="text-green-600">
              {currencySymbol} {estimate.total.toLocaleString(undefined, { maximumFractionDigits: 0 })}
            </span>
          </div>
        </div>
      )}
