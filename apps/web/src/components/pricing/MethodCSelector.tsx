...
export function MethodCSelector({
  ...
  const selectedPlating = value.platingType ? getPlatingOption(value.platingType) : null;
  ...
  const baseMetalCostNpr = value.baseMetal && weightGrams > 0 
    ? calculateBaseMetalCost(value.baseMetal, weightGrams) 
    : 0;
  const baseMetalCost = baseMetalCostNpr * nprToSelectedRate;
  
  const platingCostNpr = value.platingType && value.platingType !== 'NONE' && value.platingTier && weightGrams > 0
    ? calculatePlatingCost(value.platingType, value.platingTier, jewelleryType, weightGrams)
    : 0;
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 pb-2 border-b">
          ...
          <Tooltip>
            ...
            <TooltipContent className="max-w-[300px]">
              <p className="text-xs">
                Method C uses affordable base metals with a precious metal plating. 
                This gives the look of gold/silver at a fraction of the cost.
                <br /><br />
                <strong>Note:</strong> Plating wears over time and may need re-application.
              </p>
            </TooltipContent>
          </Tooltip>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
          ...
          <p className="mt-1 text-blue-700">
            Prices reflect coating thickness, durability, and process quality — not precious metal by the gram.
          </p>
        </div>
              ...
              <Tooltip key={metal.value}>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={`h-auto py-3 justify-start ${
                      value.baseMetal === metal.value ? 'bg-blue-500 hover:bg-blue-600 text-white' : ''
                    }`}
                    onClick={() => onChange({ ...value, baseMetal: metal.value })}
                  >
                    <div className="text-left flex-1">
                      ...
                      <div className="text-xs opacity-80 mt-0.5">{metal.description}</div>
                      <div className="text-xs opacity-70 mt-0.5">
                        {currencySymbol} {(metal.ratePerGramNpr * nprToSelectedRate).toFixed(2)}/g
                      </div>
                    </div>
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="right" className="max-w-[300px]">
                  <div className="space-y-1 text-xs">
                    <p><strong>What it is:</strong> {metal.tooltip.what}</p>
                    <p><strong>Durability:</strong> {metal.tooltip.durability}</p>
                    <p><strong>Allergy Info:</strong> {metal.tooltip.allergyRisk}</p>
                    <p><strong>Best for:</strong> {metal.tooltip.bestFor}</p>
                  </div>
                </TooltipContent>
              </Tooltip>
            ))}
          ...
          {value.baseMetal && (
            <>
              <Select 
                ...
                onValueChange={(v) => onChange({ ...value, platingType: v as PlatingTypeC })}
              >
                <SelectTrigger>
                  ...
                  {PLATING_OPTIONS.map((plating) => (
                    <SelectItem key={plating.value} value={plating.value}>
                      <div className="flex items-center justify-between w-full gap-4">
                        ...
                        <span className="text-xs text-green-600 font-medium">
                          {currencySymbol}{Math.round(plating.baseRateNpr * nprToSelectedRate).toLocaleString()}+
                        </span>
                      </div>
                    </SelectItem>
                  ))}
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-1">
                      Base metal without plating may tarnish and develop patina over time.
                      Consider adding plating for longer-lasting appearance.
                    </p>
                  </div>
                </div>
              )}
              ...
              {selectedPlating && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-amber-800">{selectedPlating.label}</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-amber-700 font-medium">What it is:</span>
                      <p className="text-amber-900">{selectedPlating.tooltip.what}</p>
                    </div>
                    <div>
                      <span className="text-amber-700 font-medium">Durability:</span>
                      <p className="text-amber-900">{selectedPlating.tooltip.durability}</p>
                    </div>
                    <div>
                      <span className="text-amber-700 font-medium">Care:</span>
                      <p className="text-amber-900">{selectedPlating.tooltip.care}</p>
                    </div>
                    <div>
                      <span className="text-amber-700 font-medium">Re-plating:</span>
                      <p className="text-amber-900">{selectedPlating.tooltip.replatFrequency}</p>
                    </div>
                  </div>
              ...
              {PLATING_TIERS.map((tier) => (
                <Tooltip key={tier.value}>
                  <TooltipTrigger asChild>
                    <Button
                      type="button"
                      variant={value.platingTier === tier.value ? 'default' : 'outline'}
                      className={`h-auto py-3 ${
                        value.platingTier === tier.value 
                          ? tier.value === 'ECONOMY' ? 'bg-gray-500 hover:bg-gray-600' 
                            : tier.value === 'STANDARD' ? 'bg-blue-500 hover:bg-blue-600'
                            : 'bg-amber-500 hover:bg-amber-600'
                          : ''
                      } ${value.platingTier === tier.value ? 'text-white' : ''}`}
                      onClick={() => onChange({ ...value, platingTier: tier.value })}
                    >
                      <div className="text-center">
                        <div className="font-medium">{tier.label}</div>
                        <div className="text-xs opacity-80">{tier.thicknessDescription}</div>
                        <div className="text-xs mt-1">
                          {tier.multiplier === 1 ? 'Standard price' : 
                           tier.multiplier < 1 ? `${((1 - tier.multiplier) * 100).toFixed(0)}% less` :
        ...
        {weightGrams > 0 && value.baseMetal && (
          <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-lg p-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="text-muted-foreground">Base metal ({weightGrams}g):</div>
              <div className="text-right font-medium">
                {currencySymbol} {baseMetalCost.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </div>
              {value.platingType && value.platingType !== 'NONE' && selectedPlating && (
                <>
                  <div className="text-muted-foreground">
                    {selectedPlating.label} ({selectedTier?.label || 'Standard'}):
                  </div>
                  <div className="text-right font-medium text-amber-600">
                    {currencySymbol} {platingCost.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </div>
                </>
              )}
              {value.platingType === 'NONE' && (
              ...
              <div className="text-blue-800 font-medium border-t pt-1">Total materials:</div>
              <div className="text-right font-bold text-blue-900 border-t pt-1">
                {currencySymbol} {totalCost.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </div>
            </div>
            <p className="text-xs text-muted-foreground italic pt-2 border-t">
              Note: This is significantly less than solid precious metal. 
              Making charges will be calculated on total.
            </p>
          </div>
        )}
        ...
        {selectedMetal?.allergyRisk === 'HIGH' && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-start gap-2">
            ...
            <div className="text-sm text-yellow-800">
              <strong>Allergy Warning:</strong> {selectedMetal.label} may cause allergic reactions 
              in some individuals. Consider 316L Stainless Steel or Titanium for sensitive skin.
            </div>
          </div>
        )}
      </div>
