...
const notificationConfig: Record<string, { 
  ...
  ORDER_PLACED: {
    ...
    formatBody: (params) => params?.itemName ? `${params.itemName} - ${params.total ? `₹${params.total.toLocaleString()}` : ''}` : 'A new order has been placed',
  },
  ORDER_CONFIRMED: {
    icon: CheckCircle2,
    color: 'text-green-500',
    formatTitle: (params) => `Order Confirmed`,
    formatBody: (params) => `Order #${params?.orderNumber || ''} has been confirmed`,
  },
  ORDER_PACKED: {
    icon: Package,
    color: 'text-indigo-500',
    formatTitle: (params) => `Order Packed`,
    formatBody: (params) => `Order #${params?.orderNumber || ''} is packed and ready`,
  },
  ORDER_SHIPPED: {
    icon: Truck,
    color: 'text-cyan-500',
  ...
  ORDER_OUT_FOR_DELIVERY: {
    ...
    formatBody: (params) => `Order #${params?.orderNumber || ''} will arrive today`,
  },
  ORDER_DELIVERED: {
    icon: CheckCircle2,
    color: 'text-green-600',
    formatTitle: (params) => `Order Delivered`,
    formatBody: (params) => `Order #${params?.orderNumber || ''} has been delivered`,
  },
  ORDER_CANCELLED: {
    icon: AlertCircle,
    color: 'text-red-500',
    formatTitle: (params) => `Order Cancelled`,
    formatBody: (params) => `Order #${params?.orderNumber || ''} has been cancelled`,
  },
  ORDER_STATUS_UPDATE: {
    icon: Package,
    color: 'text-blue-500',
    formatTitle: (params) => `Order Update`,
    formatBody: (params) => `Order #${params?.orderNumber || ''}: ${params?.status?.replace(/_/g, ' ') || 'Status updated'}`,
  },
  // RFQ notifications
  RFQ_SENT: {
    icon: MessageSquare,
    ...
    formatBody: (params) => params?.shopName ? `Sent to ${params.shopName}` : 'Your request has been sent',
  },
  RFQ_RECEIVED: {
    icon: MessageSquare,
    color: 'text-purple-500',
    formatTitle: () => `New Quote Request`,
    formatBody: (params) => params?.customerName ? `From ${params.customerName}` : 'You have a new quote request',
  },
  OFFER_RECEIVED: {
    icon: DollarSign,
    color: 'text-amber-500',
    formatTitle: (params) => `New Quote Received`,
    formatBody: (params) => params?.shopName ? `${params.shopName} sent a quote` : 'You have received a new quote',
  },
  OFFER_COUNTERED: {
    icon: DollarSign,
    color: 'text-orange-500',
    formatTitle: () => `Counter Offer`,
    formatBody: (params) => params?.amount ? `New offer: ₹${params.amount.toLocaleString()}` : 'A counter offer has been made',
  },
  OFFER_SELECTED: {
    icon: CheckCircle2,
    color: 'text-green-500',
    formatTitle: () => `Offer Selected!`,
    formatBody: (params) => params?.orderNumber ? `Order #${params.orderNumber} created` : 'Your offer has been selected',
  },
  ...
  PAYMENT_RECEIVED: {
    ...
    formatBody: (params) => params?.amount ? `₹${params.amount.toLocaleString()} received` : 'Payment has been received',
  },
  ...
  PRODUCTION_STARTED: {
    ...
    formatBody: (params) => `Order #${params?.orderNumber || ''} is now in production`,
  },
  PRODUCTION_MILESTONE: {
    icon: Hammer,
    color: 'text-indigo-500',
    formatTitle: (params) => params?.title || 'Production Update',
    formatBody: (params) => `Order #${params?.orderNumber || ''}: ${params?.milestone || 'New milestone reached'}`,
  },
  QC_COMPLETED: {
    icon: CheckCircle2,
    color: 'text-teal-500',
  ...
  BOOKING_REMINDER: {
    ...
    formatBody: (params) => params?.expiresIn ? `Expires in ${params.expiresIn}` : 'Your booking is expiring soon',
  },
  BOOKING_EXPIRED: {
    icon: AlertCircle,
    color: 'text-red-500',
  ...
  SYSTEM_ALERT: {
    icon: AlertCircle,
    color: 'text-gray-500',
    formatTitle: (params) => params?.orderNumber ? `Order #${params.orderNumber}` : 'System Alert',
    formatBody: (params) => {
      if (params?.status) return `Status updated to ${params.status.replace(/_/g, ' ')}`;
      if (params?.itemName) return `${params.itemName}${params.total ? ` - ₹${params.total.toLocaleString()}` : ''}`;
...
export function NotificationDropdown() {
  ...
  const getNotificationContent = (notification: Notification) => {
    ...
    const allParams = { ...notification.titleParams, ...notification.bodyParams };
    return {
      icon: config.icon,
      color: config.color,
        ...
        <ScrollArea className="h-[300px]">
          {notifications.length === 0 ? (
            ...
            notifications.slice(0, 10).map((notification) => {
              const { icon: Icon, color, title, body } = getNotificationContent(notification);
              ...
              const content = (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`p-1.5 rounded-full bg-gray-100 dark:bg-gray-800 ${color}`}>
