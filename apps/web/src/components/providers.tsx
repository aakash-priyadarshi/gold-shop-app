...
function MarketPreferencesSync({ children }: { children: React.ReactNode }) {
  ...
  const syncCurrencyFromGeo = usePreferencesStore(
    (state) => state.syncCurrencyFromGeo,
  );
  ...
  React.useEffect(() => {
    ...
    if (isAuthenticated) {
      console.log(
        "[MarketPreferencesSync] Authenticated user - skipping geo sync, server is source of truth",
      );
      ...
      if (validCountries.includes(detectedCountry as CountryCode)) {
        ...
        if (userCountryChoice !== "true") {
          if (prefsCountry !== detectedCountry) {
            console.log(
              "[MarketPreferencesSync] Syncing detected country:",
              detectedCountry,
              "(was:",
              prefsCountry,
              ")",
          ...
          if (
            userCurrencyChoice !== "true" &&
            expectedCurrency &&
            prefsCurrency !== expectedCurrency
          ) {
            console.log(
              "[MarketPreferencesSync] Syncing currency to match country:",
              expectedCurrency,
              "(was:",
              prefsCurrency,
              ")",
          ...
          console.log(
            "[MarketPreferencesSync] User has explicit country choice, keeping:",
            prefsCountry,
          );
          ...
          if (userCurrencyChoice !== "true") {
            ...
            if (expectedCurrency && prefsCurrency !== expectedCurrency) {
              console.log(
                "[MarketPreferencesSync] Syncing currency to user country:",
                expectedCurrency,
              );
              syncCurrencyFromGeo(expectedCurrency);
            }
