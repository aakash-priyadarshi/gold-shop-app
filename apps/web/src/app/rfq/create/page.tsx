...
export default function CreateRfqPage() {
  ...
  const fetchPriceEstimate = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPriceEstimate({
          metalCost: data.lineItems
            ?.filter((item: { category: string }) => item.category === 'METAL')
            .reduce((sum: number, item: { amount: number }) => sum + item.amount, 0) || 0,
          makingCharge: data.makingCharge || 0,
          platingCost: data.lineItems
            ?.filter((item: { category: string }) => item.category === 'FINISH')
            .reduce((sum: number, item: { amount: number }) => sum + item.amount, 0) || 0,
          gemstoneCost: data.lineItems
            ?.filter((item: { category: string }) => ['GEMSTONE', 'SETTING'].includes(item.category))
            .reduce((sum: number, item: { amount: number }) => sum + item.amount, 0) || 0,
          ...
          breakdown: {
            metalType: formData.metalType,
            weightGrams: weight,
            ratePerGram: marketRates?.metals?.[formData.metalType as keyof typeof marketRates.metals] || 0,
  ...
  const removeGemstone = (index: number) => {
    updateFormData('gemstones', formData.gemstones.filter((_, i) => i !== index));
  };
  ...
  const isVermeilValid = () => {
    if (formData.platingType === 'VERMEIL' && formData.metalType !== 'SILVER_925') {
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    // Validate Vermeil
    if (!isVermeilValid()) {
      setError('Vermeil plating requires Sterling Silver (925) as the base metal.');
      return;
    }
    ...
    try {
      const response = await fetch(`${API_URL}/rfq`, {
        ...
        body: JSON.stringify({
          ...
          addGoldPlating: formData.buildMethod === 'METHOD_C' ? formData.addGoldPlating : false,
          platingType: formData.addGoldPlating ? formData.platingType : undefined,
          platingTier: formData.addGoldPlating ? formData.platingTier : undefined,
          surfaceFinish: formData.surfaceFinish,
          // Structured gemstones
          gemstones: formData.hasGemstones && formData.gemstones.length > 0 
              ...
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex items-center">
                  ...
                  <span className={`ml-2 hidden sm:inline ${step >= s ? 'text-gray-900' : 'text-gray-500'}`}>
                    {s === 1 ? 'Basic Info' : s === 2 ? 'Design Details' : 'Budget & Timeline'}
                  </span>
                  {s < 3 && (
                    <div className={`w-16 sm:w-24 h-1 mx-4 ${step > s ? 'bg-gold-500' : 'bg-gray-200'}`} />
                  )}
                </div>
              ))}
            </div>
              ...
              <CardContent className="space-y-6">
                ...
                {formData.jewelleryType && templates.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">
                      Templates provide pre-defined weight ranges and recommended materials
                    </p>
                  </div>
                )}
                        ...
                        return (
                          <Button
                            ...
                            className={`flex-1 flex-col h-auto py-3 ${
                              formData.weightCategory === cat.value ? 'gold-gradient text-white' : ''
                            }`}
                ...
                {formData.buildMethod === 'METHOD_A' && (
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v: string) => updateFormData('metalType', v)}
                    >
                      <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v: string) => updateFormData('surfaceFinish', v)}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {surfaceFinishes.length > 0 ? (
                        ...
                        <>
                          <SelectItem value="POLISHED">High Polish / Mirror Finish</SelectItem>
                          <SelectItem value="MATTE">Matte / Brushed Finish</SelectItem>
                          <SelectItem value="SATIN">Satin Finish</SelectItem>
                          <SelectItem value="HAMMERED">Hammered Texture</SelectItem>
                          <SelectItem value="SANDBLAST">Sandblasted</SelectItem>
                          <SelectItem value="ANTIQUE">Antique / Oxidized</SelectItem>
                        </>
                      )}
                    </SelectContent>
                  </Select>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
                  ...
                  <div className="text-sm text-blue-700">
                    ...
                    <p>
                      After submitting your request, multiple verified jewellers will
                      review your requirements and send you competitive quotes. You can
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => updateFormData('specialInstructions', e.target.value)}
                  />
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateFormData('budgetMin', e.target.value)}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateFormData('budgetMax', e.target.value)}
                    />
                  </div>
                </div>
                ...
                {priceEstimate && priceEstimate.total != null && 
                 (priceEstimate.total || 0) > parseFloat(formData.budgetMax || '0') && formData.budgetMax && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4 text-amber-600" />
                    <p className="text-sm text-amber-700">
                      Estimated price exceeds your maximum budget. Consider adjusting specifications or budget.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateFormData('deadline', e.target.value)}
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <span>{getAvailableMetals().find((t) => t.id === formData.metalType)?.name}</span>
                    ...
                    {formData.surfaceFinish && (
                      <>
                        ...
                        <span>{formData.surfaceFinish.replace('_', ' ')}</span>
                      </>
                    )}
                    ...
                    {formData.addGoldPlating && (
                      <>
                        ...
                        <span>{formData.platingType.replace('_', ' ')} ({formData.platingTier})</span>
                      </>
                    )}
                ...
                {formData.buildMethod === 'METHOD_C' && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex gap-2">
                    ...
                    <p className="text-sm text-amber-700">
                      <strong>Note:</strong> This piece uses base metal with plating/coating. 
                      It is not solid gold and will be labeled accordingly.
                    </p>
                  </div>
                )}
