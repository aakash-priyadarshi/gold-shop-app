...
function WalkInRfqModal({
  ...
  const [measurements, setMeasurements] = useState({ ringSize: "", wristSize: "", chainLength: "", bangleSize: "" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: err.response?.data?.message || "Failed to create walk-in RFQ" });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Step {step} of 3 —{" "}
              {step === 1 ? "Select Items" : step === 2 ? "Request Details" : "Customer Info"}
            </p>
          </div>
        ...
        <div className="flex gap-1 px-4 pt-3">
          {[1, 2, 3].map((s) => (
            <div key={s} className={`h-1 flex-1 rounded-full ${s <= step ? "bg-gold-500" : "bg-gray-200 dark:bg-gray-700"}`} />
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {step === 1 && (
            <>
              <p className="text-sm text-gray-600 dark:text-gray-400">Selected items from showroom session:</p>
              {selectedItems.length === 0 && (
                <p className="text-sm text-gray-400 text-center py-6">No items selected.</p>
              )}
              {selectedItems.map((si, idx) => (
                <div key={si.item.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                  {si.item.inventoryItem.images?.[0] ? (
                    <img src={si.item.inventoryItem.images[0]} className="w-12 h-12 rounded-lg object-cover" alt="" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {si.item.inventoryItem.purity ? ` · ${si.item.inventoryItem.purity}` : ""}
                    </div>
                    {/* Variant selector */}
                    {si.item.inventoryItem.variants && si.item.inventoryItem.variants.length > 0 && (
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => updateQty(idx, si.qty - 1)} className="w-6 h-6 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 flex items-center justify-center text-xs">-</button>
                    <span className="w-6 text-center text-sm text-gray-900 dark:text-white">{si.qty}</span>
                    <button onClick={() => updateQty(idx, si.qty + 1)} className="w-6 h-6 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 flex items-center justify-center text-xs">+</button>
                  </div>
          ...
          {step === 2 && (
            <>
              <div>
                ...
                <select value={jewelleryType} onChange={(e) => setJewelleryType(e.target.value)} className={inputClass}>
                  <option value="">Select type (optional)</option>
                  {["RING", "NECKLACE", "BRACELET", "BANGLE", "EARRING", "PENDANT", "CHAIN", "ANKLET", "BROOCH", "OTHER"].map((t) => (
              ...
              <div>
                ...
                <input type="number" value={deadlineDays} onChange={(e) => setDeadlineDays(e.target.value)} placeholder="e.g. 14" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Notes</label>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="Special instructions, engraving, etc." className={inputClass} />
              </div>
              <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mt-2">Measurements (optional)</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelClass}>Ring Size</label>
                  <input value={measurements.ringSize} onChange={(e) => setMeasurements({ ...measurements, ringSize: e.target.value })} placeholder="e.g. 7" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Wrist Size</label>
                  <input value={measurements.wristSize} onChange={(e) => setMeasurements({ ...measurements, wristSize: e.target.value })} placeholder="e.g. 6.5 inches" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Chain Length</label>
                  <input value={measurements.chainLength} onChange={(e) => setMeasurements({ ...measurements, chainLength: e.target.value })} placeholder="e.g. 18 inches" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Bangle Size</label>
                  <input value={measurements.bangleSize} onChange={(e) => setMeasurements({ ...measurements, bangleSize: e.target.value })} placeholder="e.g. 2.4" className={inputClass} />
                </div>
              </div>
            </>
          )}
          ...
          {step === 3 && (
            <>
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-xs text-amber-700 dark:text-amber-400">
                This info is stored for shop internal follow-up only. It is never shared in platform chat or visible to buyers.
              </div>
              <div>
                <label className={labelClass}>Customer Name</label>
                <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Customer Phone</label>
                <input value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Internal Notes</label>
                <textarea value={customerNotes} onChange={(e) => setCustomerNotes(e.target.value)} rows={2} placeholder="Any internal notes about this customer" className={inputClass} />
              </div>
            </>
          )}
        </div>
        ...
        {walkInModal && (
          <WalkInRfqModal
            ...
            onSuccess={(rfqId) => {
              ...
              toast({ title: `Walk-in RFQ created!${rfqId ? ` ID: ${rfqId.slice(0, 8)}...` : ""}` });
