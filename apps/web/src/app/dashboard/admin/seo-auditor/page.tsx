...
export default function SeoAuditorPage() {
  ...
  const generateMarkdownReport = (currentReport: SeoAuditReport): string => {
    ...
    md += `**Overall Platform Score:** ${currentReport.overallScore} / 100\n\n`;

    md += `## Executive Summary\n`;
    md += `- **Total Pages Scanned:** ${currentReport.totalPages}\n`;
    md += `- **Healthy / Indexable Pages:** ${currentReport.indexablePages}\n`;
    md += `- **Redirected Pages:** ${currentReport.redirectPages}\n`;
    md += `- **Warning Pages:** ${currentReport.warningPages}\n`;
    md += `- **Error / Broken Pages:** ${currentReport.errorPages}\n`;
    md += `- **Critical Auth Redirect Faults:** ${currentReport.criticalRedirects}\n\n`;

    if (currentReport.criticalRedirects > 0) {
      md += `> 🚨 **CRITICAL WARNING:** There are ${currentReport.criticalRedirects} public pages redirecting crawler bots to the login screen (\`/auth/login\`). This will actively prevent Google and other search engines from indexing your primary marketing pages!\n\n`;
    }

    md += `## Detailed Page-by-Page Audit\n\n`;
    md += `| Page Path | Score | Status | Primary Title | Recommendations Count |\n`;
    md += `| :--- | :---: | :---: | :--- | :---: |\n`;
    
    currentReport.pages.forEach((page) => {
      const statusEmoji = page.status === "SUCCESS" ? "✅" : page.status === "WARNING" ? "⚠️" : "❌";
      const cleanTitle = page.title ? page.title.replace(/\|/g, "\\|") : "N/A";
      md += `| \`${page.path}\` | ${page.score}% | ${statusEmoji} ${page.status} | ${cleanTitle} | ${page.recommendations.length} |\n`;
    });

    md += `\n## Full Analysis & Content Optimizations\n\n`;
    currentReport.pages.forEach((page) => {
      const statusEmoji = page.status === "SUCCESS" ? "✅" : page.status === "WARNING" ? "⚠️" : "❌";
      md += `### ${statusEmoji} Path: \`${page.path}\` (Score: ${page.score}%)\n`;
      md += `- **Page Title:** ${page.title || "N/A"}\n`;
      md += `- **Meta Description:** ${page.description || "N/A"}\n`;
      ...
      md += `- Googlebot Desktop: HTTP ${page.crawlResults.googlebotDesktop.status}${page.crawlResults.googlebotDesktop.redirectTarget ? ` -> redirects to ${page.crawlResults.googlebotDesktop.redirectTarget}` : ""}\n`;
      md += `- Browser Desktop: HTTP ${page.crawlResults.browserDesktop.status}${page.crawlResults.browserDesktop.redirectTarget ? ` -> redirects to ${page.crawlResults.browserDesktop.redirectTarget}` : ""}\n`;
      md += `- Browser Mobile: HTTP ${page.crawlResults.browserMobile.status}${page.crawlResults.browserMobile.redirectTarget ? ` -> redirects to ${page.crawlResults.browserMobile.redirectTarget}` : ""}\n\n`;
      ...
      if (page.recommendations.length > 0) {
        ...
        md += `*No errors or warning alerts encountered for this route. Good job!*\n\n`;
      }
      md += `---\n\n`;
    });

    md += `\n*Report automatically generated by Orivraa Admin SEO Auditor & Content Optimizer Bot.*`;
    return md;
  };
  ...
  const copyReportToClipboard = () => {
    ...
    try {
      ...
      toast({
        ...
        description: "SEO report copied as structured Markdown successfully.",
      });
    } catch (err) {
      toast({
        title: "Copy Failed",
        description: "Could not write to the clipboard. Please try again.",
        variant: "destructive",
      });
    }
  };

  const generatePagesTableHtml = (pages: SeoAuditPageReport[]): string => {
    let html = "";
    pages.forEach((page) => {
      html += "<tr>";
      html += "<td><code>" + page.path + "</code></td>";
      html += "<td>" + page.score + "%</td>";
      html += "<td>" + page.status + "</td>";
      html += "<td>" + (page.title || "N/A") + "</td>";
      html += "<td>" + page.recommendations.length + "</td>";
      html += "</tr>";
    });
    return html;
  };

  const generatePagesOptimizationsHtml = (pages: SeoAuditPageReport[]): string => {
    let html = "";
    pages.forEach((page) => {
      html += "<h3>Path: <code>" + page.path + "</code> (Score: " + page.score + "%)</h3>";
      html += "<ul>";
      html += "<li><strong>Page Title:</strong> " + (page.title || "N/A") + "</li>";
      html += "<li><strong>Meta Description:</strong> " + (page.description || "N/A") + "</li>";
      html += "<li><strong>Canonical Link:</strong> " + (page.canonical || "N/A") + "</li>";
      html += "<li><strong>H1 Tag Count:</strong> " + page.h1Count + "</li>";
      html += "<li><strong>Word Count:</strong> " + page.wordCount + " words</li>";
      html += "<li><strong>Robots Meta-Tag:</strong> " + (page.robots || "N/A") + "</li>";
      html += "</ul>";

      html += "<h4>Simulated Crawl Results:</h4>";
      html += "<ul>";
      html += "<li>Googlebot Mobile: HTTP " + page.crawlResults.googlebotMobile.status + (page.crawlResults.googlebotMobile.redirectTarget ? " (redirects to " + page.crawlResults.googlebotMobile.redirectTarget + ")" : "") + "</li>";
      html += "<li>Googlebot Desktop: HTTP " + page.crawlResults.googlebotDesktop.status + (page.crawlResults.googlebotDesktop.redirectTarget ? " (redirects to " + page.crawlResults.googlebotDesktop.redirectTarget + ")" : "") + "</li>";
      html += "<li>Browser Desktop: HTTP " + page.crawlResults.browserDesktop.status + (page.crawlResults.browserDesktop.redirectTarget ? " (redirects to " + page.crawlResults.browserDesktop.redirectTarget + ")" : "") + "</li>";
      html += "<li>Browser Mobile: HTTP " + page.crawlResults.browserMobile.status + (page.crawlResults.browserMobile.redirectTarget ? " (redirects to " + page.crawlResults.browserMobile.redirectTarget + ")" : "") + "</li>";
      html += "</ul>";

      if (page.recommendations.length > 0) {
        html += "<h4>Actionable Recommendations:</h4>";
        html += "<ul>";
        page.recommendations.forEach((rec) => {
          html += "<li>" + rec.replace(/\*\*/g, "") + "</li>";
        });
        html += "</ul>";
      } else {
        html += "<p style='font-size:12px;'><em>No errors or warning alerts encountered for this route. Good job!</em></p>";
      }
  ...
  const downloadReportFile = (format: "md" | "txt" | "doc") => {
    ...
    if (format === "md") {
      ...
      filename += ".txt";
      mimeType = "text/plain";
      const rawMd = generateMarkdownReport(report);
      content = rawMd
        .replace(/\|/g, " ")
        .replace(/#+\s+/g, "")
        .replace(/[-*]\s+/g, "• ")
      ...
      content = `
        ...
        <head><title>SEO Audit Executive Report</title>
        <style>
          body { font-family: 'Segoe UI', Arial, sans-serif; line-height: 1.6; color: #333; }
          h1 { color: #d97706; border-bottom: 2px solid #d97706; padding-bottom: 8px; font-size: 24px; }
          h2 { color: #1f2937; border-bottom: 1px solid #e5e7eb; padding-bottom: 6px; margin-top: 24px; font-size: 18px; }
          h3 { color: #4b5563; margin-top: 18px; font-size: 14px; }
          table { border-collapse: collapse; width: 100%; margin: 16px 0; }
          th, td { border: 1px solid #d1d5db; padding: 8px 12px; text-align: left; font-size: 12px; }
          th { background-color: #f3f4f6; font-weight: bold; }
          code { font-family: Consolas, monospace; background-color: #f3f4f6; padding: 2px 4px; border-radius: 4px; font-size: 11px; }
          .critical { color: #dc2626; font-weight: bold; background-color: #fee2e2; padding: 8px; border-left: 4px solid #dc2626; margin: 10px 0; }
          ul { margin: 8px 0; padding-left: 20px; }
          li { font-size: 12px; margin-bottom: 4px; }
        </style>
        </head>
        <body>
          <h1>Orivraa SEO Auditor Bot - Executive Crawl Report</h1>
          <p style="font-size:12px;"><strong>Date of Audit:</strong> ${new Date(report.timestamp).toLocaleString()}</p>
          <p style="font-size:12px;"><strong>Target URL:</strong> ${targetHost}</p>
          <p style="font-size:12px;"><strong>Overall Platform Score:</strong> ${report.overallScore} / 100</p>
          
          <h2>Executive Summary</h2>
          <ul>
            <li>Total Pages Scanned: ${report.totalPages}</li>
            <li>Healthy / Indexable Pages: ${report.indexablePages}</li>
            <li>Redirected Pages: ${report.redirectPages}</li>
            <li>Warning Pages: ${report.warningPages}</li>
            <li>Error / Broken Pages: ${report.errorPages}</li>
            <li>Critical Auth Redirect Faults: ${report.criticalRedirects}</li>
          </ul>
          
          ${report.criticalRedirects > 0 ? `
            <div class="critical">
              🚨 <strong>CRITICAL WARNING:</strong> There are ${report.criticalRedirects} public pages redirecting crawler bots to the login screen (/auth/login). This will actively prevent Google and other search engines from indexing your primary marketing pages!
            </div>
          ` : ""}
          
          <h2>Detailed Page-by-Page Audit</h2>
          <table>
            <thead>
              <tr>
                <th>Page Path</th>
            ...
            <tbody>
              ${generatePagesTableHtml(report.pages)}
            </tbody>
          </table>
          
          <h2>Full Analysis & Content Optimizations</h2>
          ${generatePagesOptimizationsHtml(report.pages)}
        </body>
        </html>
      `;
    }
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-9 rounded-lg border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 touch-target"
                    >
                  ...
                  <DropdownMenuContent align="end" className="w-56 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-1 z-[100]">
                    <DropdownMenuLabel className="text-xs text-gray-400 px-3 py-2">Select Export Format</DropdownMenuLabel>
                    <DropdownMenuSeparator className="my-1 border-gray-100 dark:border-gray-800" />
                    <DropdownMenuItem
                      onClick={copyReportToClipboard}
                      className="flex items-center gap-2 px-3 py-2 text-sm rounded-lg hover:bg-amber-50 dark:hover:bg-amber-950/20 text-gray-700 dark:text-gray-200 cursor-pointer focus:bg-amber-50 dark:focus:bg-amber-950/20 outline-none"
                    >
                    ...
                    <DropdownMenuItem
                      onClick={() => downloadReportFile("doc")}
                      className="flex items-center gap-2 px-3 py-2 text-sm rounded-lg hover:bg-amber-50 dark:hover:bg-amber-950/20 text-gray-700 dark:text-gray-200 cursor-pointer focus:bg-amber-50 dark:focus:bg-amber-950/20 outline-none"
                    >
                      <FileCheck2 className="h-4 w-4 text-amber-500" />
