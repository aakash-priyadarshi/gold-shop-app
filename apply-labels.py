...
replacements = [
    (
        r'''<div className="grid grid-cols-[100px_1fr] gap-2">
    ...
    (
        r'''        <section className="space-y-3 rounded-2xl border border-gray-100 bg-white p-4">
    ...
    (
        r'''          {gemstoneRateOptions.length > 0 && (
...
for idx, (old, nv) in enumerate(replacements):
    if old not in text:
        print(f"Failed to find block {idx}")
    else:
        text = text.replace(old, nv)
